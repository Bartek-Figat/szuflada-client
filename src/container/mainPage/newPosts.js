    ...
    <div className="flex h-full w-full divide-x divide-gray-500 mt-20">
      <div className="flex m-auto">
        <div className="grid sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {data.map((items) => (
            <div className="flex flex-col bg-white w-96 h-96 rounded shadow transition-all duration-200 hover:shadow-downfall-lg transform hover:-translate-y-0.5">
              <div className="w-full aspect-w-4 aspect-h-3 rounded overflow-hidden">
                <Link to="#">
                  <img class="w-full h-full object-cover object-center" src={items.image} alt="" />
                </Link>
              </div>
              <div className="flex">
